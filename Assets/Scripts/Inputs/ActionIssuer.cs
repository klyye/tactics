...
public static class ActionIssuer
{
    ...
    public static void IssueMove(Actor doer, Vector2Int target)
    {
        var mover = doer.GetComponent<Mover>();
        var valid = mover && !doer.locked && tm.currentPlayer.Controls(doer);
        if (!valid) return;
        var start = gm.grid.PositionToCoord(mover.transform.position);
        var path = Pathfinder.ShortestPath(start, target, doer.actionPoints);
        if (path != null)
    ...
    public static void IssueAttack(Actor doer, Selectable target)
    {
        ...
        var valid = tm.currentPlayer.Controls(doer) && atker && defnder &&
                    doercrds.ManhattanDist(targetcrds) < atker.atkRange && !doer.locked;
        if (!valid) return;
